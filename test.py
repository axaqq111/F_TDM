...
def evaluate(csv_path: str  = "AirQualityUCI.csv",
    ...
    RMSE on the **original NOx scale** for each few-shot setting.
    ...
    print(" (RMSE reported on original NOx scale)")
    print("=" * 60)
    ...
    for k in few_shot_k:
        ...
        # Support set: first k groups (normalised)
        sup_W = to_tensor(nox_W[:k])
        sup_U = to_tensor(nox_U[:k])

        # Test set: the next n_test groups (human participant data only)
        test_W = to_tensor(nox_W[k: k + n_test])
        test_U_norm = nox_U[k: k + n_test]   # normalised, kept as numpy

        if len(test_W) == 0:
            print(f"{k:>10} | {k:>18} | {'N/A':>13} | {'N/A':>10}")
            continue

        # Fine-tune on the k support samples (in normalised space)
        ft_model = fine_tune(model, sup_W, sup_U,
                             n_steps=fine_tune_steps, lr=ALPHA)

        # Predict on test set (human participant data only, normalised)
        ft_model.eval()
        with torch.no_grad():
            pred_norm = ft_model(test_W).squeeze(-1).numpy()

        # Inverse-transform both predictions and ground truth to original scale
        # so that RMSE is on the same scale reported in the paper (~55)
        ...
        # Compute RMSE on the original NOx scale
        error = rmse(pred_orig, test_U_orig)

        results[k] = error
        print(f"{k:>10} | {k:>18} | {len(test_W):>13} | {error:>10.4f}")

    print("=" * 60)
    print("\nSummary: RMSE per few-shot setting (original NOx scale)")
    for k, r in results.items():
        print(f"  k = {k:2d} PoIs  →  RMSE = {r:.4f}")
